    ...
    <div class="select-option-box" ref="selectBox">
      <div class="select-option-list">
        <template v-for="item in data">
          <div v-if="!item.hidden && item.label.indexOf(inputLabel) !== -1" :key="item.value" class="select-option"
               @click="clickOption(item)">
            <span :class="{'select-option-on': item.value === inputValue}"
                  v-html="item.label.replace(inputLabel, `<span style='color: red;'>${inputLabel}</span>`)"></span>
          </div>
        </template>
      </div>
    </div>
